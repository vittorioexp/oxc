...
/// NOT by pre-attaching to nodes like Prettier.
///
...
/// over-report blank lines (e.g. an indicator such as `-` sitting on its own line).
///
/// The source is normalized to `\n` before parsing (see `format()`), but the
/// CR-handling is kept so the helper stays correct on raw slices in tests.
...
/// Mirrors Prettier's comment printing: `"#" + comment.value` where the value
/// keeps the original spacing after `#`.
...
/// The start of the LAST pending comment up to `before`, when it is a
/// suppression marker. Callers flushing a block collection's leading comments
/// bound the flush here so the marker survives for the first item's own check
/// (an ignore right above the first item freezes that item, not the whole
/// collection).
pub fn suppression_marker_start_before(f: &YamlFormatter<'_, '_>, before: u32) -> Option<u32> {
    let source = f.context().source_text();
    f.context()
        .comments()
        .iter_before(before)
