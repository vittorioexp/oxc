...
/// (see `prettier/src/language-yaml/options.js`; `trailingComma` is consumed
/// by the flow-collection printer). Other Prettier options have no effect on
/// YAML output.
#[derive(Debug, Default, Clone, Copy, Eq, PartialEq)]
pub struct YamlFormatOptions {
    /// Present to satisfy [`FormatOptions`], but a no-op for output: YAML forbids tab
    /// indentation, so Prettier's YAML printer routes ALL indentation through
    /// `alignWithSpaces` (string-form `align`, which `useTabs` never converts to tabs).
    /// This crate mirrors that by printing exclusively with `align()`, never `indent()` —
    ...
    /// (`es5`/`all` allow it, `none` forbids it; YAML makes no finer distinction).
