...
/// formatter's document (dispatcher path, e.g. a fenced ```` ```yaml ```` block in markdown).
///
/// Unlike [`format()`], this:
...
/// - a leading BOM is stripped first so offset-based scans never see it
///   (the caller re-emits it from `has_bom`)
/// - CRLF / lone CR are normalized to `\n`: scalars are printed as raw source
///   slices throughout, and the core IR forbids raw `\r` in text — normalizing
///   once here keeps every slice site (`split('\n')`, column/gap byte scans)
///   free of CR handling; the printer re-emits the configured `end_of_line`
///   at the final stage
fn parse_root<'a>(
    allocator: &'a Allocator,
    source_text: &str,
...
/// Emits the stream's documents followed by any trailing comments,
...
impl<'a> Format<'a, YamlFormatContext<'a>> for FormatYamlRoot<'a> {
    fn fmt(&self, f: &mut YamlFormatter<'_, 'a>) {
        ...
        // Prettier suppresses this when the stream's last descendant is a
        // keep-chomped (`+`) block scalar, whose verbatim content already
        // carries the trailing newlines.
