...
/// Emits a quoted scalar with Prettier's quote normalization
/// (`printer-yaml.js` `quoteDouble`/`quoteSingle` case).
pub fn write_quoted(kind: FlowScalarKind, span: Span, f: &mut YamlFormatter<'_, '_>) {
    ...
    if raw.contains('"') {
        ...
        if kind == FlowScalarKind::QuoteDouble {
            ...
            // (The guarding `contains('"')` means the first replace always
            // matches, so the chain is owned; copy it into the arena once.)
...
/// Prettier's `printFlowScalarContent`: each reflow group becomes a
/// `fill(join(line, words))`, groups joined by hardlines.
///
/// Empty groups (blank lines) are emitted as raw `\n` text so consecutive
/// breaks survive the printer's newline collapsing (same technique as block
/// scalars); a following hardline only re-arms indentation.
pub fn write_flow_scalar_content<'a>(
    ...
    // Fast path: a single-line scalar under `preserve` (the default) prints
    // verbatim — skip the line-contents machinery and its allocations.
    ...
    for words in &line_groups {
        ...
        if blanks > 0 {
            // N blank groups = N separators before this group (plus the one
            // separating it from the previous non-empty group, if any).
...
/// Prettier's `getFlowScalarLineContents`: position-dependent trimming, then
/// paragraph merging (unless `preserve`), then `never` collapses each
...
/// Prettier's `splitWithSingleSpace` (`/(?<!^| ) (?! |$)/`): split on single
/// spaces that are not at a boundary and not adjacent to another space, so
/// multi-space runs stay inside one "word". Empty input yields no words;
...
pub fn split_with_single_space(text: &str) -> impl Iterator<Item = &str> {
    ...
    std::iter::from_fn(move || {
        ...
        // A split position needs a non-space on both sides, so resuming at
        // `start + 1` (skipping the first byte of the new word) is safe.
