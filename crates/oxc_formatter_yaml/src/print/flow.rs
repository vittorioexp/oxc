...
use crate::{
    ...
    print::{
        YamlFormatter, content_start, format_with, mapping_item, mapping_item_start, write_content,
    },
};

/// Ports Prettier's `printFlowMapping` / `printFlowSequence`
/// (`print/flow-mapping-sequence.js`). The caller wraps the result in a group.
pub fn write_flow_mapping<'a>(flow: &'a FlowMapping<'a>, f: &mut YamlFormatter<'_, 'a>) {
    let is_empty_last_item = flow
        .children
        .last()
        .is_some_and(|item| item.key.content.is_none() && item.value.content.is_none());
    ...
    let inner = format_with(|f: &mut YamlFormatter<'_, 'a>| {
        ...
        for (i, item) in flow.children.iter().enumerate() {
            ...
            if i + 1 < count {
                write_entry_separator(item.span.end, mapping_item_start(&flow.children[i + 1]), f);
            }
        }
        ...
        for (i, entry) in flow.children.iter().enumerate() {
            ...
            if is_suppressed_last_before(f, start) {
                ...
                let entry_content = format_with(|f: &mut YamlFormatter<'_, 'a>| match entry {
                    FlowSequenceEntry::Item(item) => write_content(&item.content, f),
...
fn flow_entry_bounds(entry: &FlowSequenceEntry<'_>) -> (u32, u32) {
    match entry {
        FlowSequenceEntry::Item(item) => (content_start(&item.content), item.span.end),
        FlowSequenceEntry::Pair(item) => (mapping_item_start(item), item.span.end),
    }
}
...
/// `line` between entries, plus an extra newline when the source had a blank
/// line (Prettier's `printNextEmptyLine` inside flow children).
///
/// The blank line must appear only when the collection breaks and must not
/// force it to break, so it is a raw `\n` behind `if_group_breaks` with
/// expand-propagation suppressed (the following `line` re-arms indentation:
/// after the raw newline the printer sees an empty line and emits none itself).
fn write_entry_separator(prev_end: u32, next_start: u32, f: &mut YamlFormatter<'_, '_>) {
    ...
    if anchor > prev_end {
        ...
        if classify_gap(gap) == Gap::Blank {
            // The raw text supplies BOTH newlines; the following `line` then
            // finds itself at a fresh line and only re-arms indentation.
...
/// Trailing comma when broken, then any comments left before the closing
/// bracket as own-line end comments.
