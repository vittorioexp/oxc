...
use oxc_yaml_parser::ast::{
    Chomping, Content, Directive, Document, Mapping, MappingItem, Props, Root, Sequence,
};

use crate::{
    comments::{
        Gap, classify_gap, flush_leading_comments, gap_upper_bound, is_suppressed_last_before,
        suppression_marker_start_before, write_single_comment, write_suppressed_node,
...
/// Wraps a re-entrant YAML closure in a [`FormatWith`]. The closure's context is
/// pinned to [`YamlFormatContext`] so call sites don't have to annotate it.
#[inline]
...
/// The significant start of a content node, including its anchor/tag properties
/// (the parser attaches props to the node but does not widen the node's span).
pub fn content_start(content: &Content<'_>) -> u32 {
    let props = content.props();
    let mut start = content.span().start;
    if let Some(anchor) = props.anchor {
        start = start.min(anchor.span.start);
    }
    if let Some(tag) = props.tag {
        start = start.min(tag.span.start);
    }
    start
}

/// The significant start of a mapping item, including the key content's props.
pub fn mapping_item_start(item: &MappingItem<'_>) -> u32 {
    let mut start = item.span.start;
    if let Some(key) = &item.key.content {
        start = start.min(content_start(key));
    }
    start
}
...
/// A run of `count` newlines with the arena lifetime. Small runs (the
/// overwhelming case: blank-line counts) slice a static string instead of
/// building a `String` and copying it into the arena.
...
/// Returns `true` when the stream's last descendant is a keep-chomped (`+`)
/// block scalar. Its verbatim content already ends with the kept newlines, so
/// the caller must not append the usual final `hard_line_break()`
/// (mirrors Prettier's `shouldPrintHardline` gate in `printer-yaml.js`).
pub fn ends_with_keep_chomped_block(root: &Root<'_>) -> bool {
    root.children.last().and_then(|document| document.body.content.as_ref()).is_some_and(
        |content| {
            last_descendant_block_scalar(content)
                .is_some_and(|block| block.chomping == Chomping::Keep)
        },
    )
}

/// The block scalar the content's last descendant resolves to, if any.
/// Its span consumes the trailing line breaks, so "same line" checks against
/// the content end are meaningless after one.
fn last_descendant_block_scalar<'b>(
    ...
    match content {
        ...
        Content::Mapping(mapping) => mapping
            .children
            .last()
            .and_then(|item| item.value.content.as_ref())
...
/// Top-level stream: documents joined by hard lines with `---`/`...` marker
/// logic, then any comments trailing the last document.
///
/// Returns whether the stream ends with a keep-chomped block scalar (computed
/// here anyway) so `format()` doesn't re-walk the tree for its final newline.
...
/// The gap-measurement anchor after a document: its `...` marker, or the end
/// of its body (adjusted for a block scalar tail, whose span consumes the
/// trailing line breaks).
fn document_gap_anchor(document: &Document<'_>, f: &YamlFormatter<'_, '_>) -> u32 {
    document.document_end_marker.map_or_else(
        || item_gap_anchor(document.body.content.as_ref(), document.body.span.end, f),
        |marker| marker.end,
    )
}
...
///
/// NOT PORTED (non-follow): whether Prettier keeps a blank line here depends
/// on the previous body's node kind and on what follows (`printNextEmptyLine`
/// covers block collections only; its `documentBody` end-comments separator
/// covers block mappings, and unconditionally INSERTS one after a non-keep
/// block scalar) — the inconsistency tracked as prettier#15528. The consistent
/// rule instead: a blank line in the source is preserved (normalized to one),
/// never invented, for every node kind alike.
...
/// Writes a document body's end comments, one per line, preserving the
/// source's blank lines (normalized to one) — in front of the first comment
/// (measured from `anchor`) and between consecutive comments alike.
...
/// Prettier's `shouldPrintDocumentEndMarker`: an explicit `...`, or a following
/// document whose head has directives or leading comments (which would
/// otherwise be ambiguous without the marker).
fn should_print_document_end_marker(document: &Document<'_>, next: Option<&Document<'_>>) -> bool {
    ...
    // NOTE: Prettier also checks `hasEndComments(nextDocument.head)`, but a
    // comment only lands in the next head when THIS document has a `...`
    // marker (already handled above); without one it belongs to this
    // document's body, so no `...` is introduced for it.
...
fn write_document<'a>(document: &'a Document<'a>, f: &mut YamlFormatter<'_, 'a>) {
    ...
    if let Some(content) = &document.body.content {
        if needs_line {
            write!(f, hard_line_break());
        }
        let start = content_start(content);
        // A block collection starts at the same offset as its first item; the
        // suppression marker is left for the item loop, so only the FIRST ITEM
        // is frozen — an ignore right above the first key must not silently
        // freeze the whole document. Whole-body suppression needs the explicit
        // head form (`# oxfmt-ignore` above `---`).
        let body_is_block_collection =
            matches!(content, Content::Mapping(_) | Content::Sequence(_));
        if head_ignores_body || (!body_is_block_collection && is_suppressed_last_before(f, start)) {
            write_suppressed_node(to_span(content.span()), f);
        } else {
            flush_leading_comments(suppression_flush_bound(body_is_block_collection, start, f), f);
            write_content(content, f);
            // A block scalar's span consumes its trailing breaks; a comment on
            // the next line would look same-line, but none can follow one.
            if last_descendant_block_scalar(content).is_none() {
                write_trailing_same_line_comment(content.span().end, f);
            }
        }
    }
}

/// Prettier re-joins directive words with single spaces: `["%", join(" ", [name, ...params])]`.
...
/// Prettier normalizes to `tag anchor` order; we keep the source order instead
/// (Prettier bug #19524 — reordering is not worth reproducing, and preserving
/// input order is the safe non-reordering behavior).
///
/// The separator after the props is a hard line for block collections and a
/// space otherwise (`printer-yaml.js:83-89`).
...
pub fn write_content<'a>(content: &'a Content<'a>, f: &mut YamlFormatter<'_, 'a>) {
    let is_block_collection = matches!(content, Content::Mapping(_) | Content::Sequence(_));
    // Middle comments (between the props and the node body, `&a # c`) keep the
    // props separator a space even for block collections (`printer-yaml.js:84`).
    let has_middle_comments =
        f.context().comments().peek().is_some_and(|c| c.end <= content.span().start);
    write_props(content.props(), is_block_collection && !has_middle_comments, f);
    ...
    let middles_bound = suppression_flush_bound(is_block_collection, content.span().start, f);
    ...
    match content {
        ...
        Content::Mapping(mapping) => write_mapping(mapping, f),
...
/// Wraps a flow collection in its group, then claims any comments the flow
/// printer's bounded flushes missed so they aren't re-emitted later.
...
/// Flush bound for a block collection's leading comments: stops before a
/// trailing suppression marker so the first item's own check claims it
/// (an ignore right above the first item freezes that item, not the whole
/// collection).
fn suppression_flush_bound(
...
pub fn last_descendant_end(root: &Root<'_>) -> u32 {
    fn content_end(content: &Content<'_>) -> u32 {
        match content {
            Content::Mapping(mapping) => mapping.children.last().map_or(mapping.span.end, |item| {
                item.value.content.as_ref().map_or(item.span.end, content_end)
            }),
            Content::Sequence(sequence) => {
                sequence.children.last().map_or(sequence.span.end, |item| {
                    item.content.as_ref().map_or(item.span.end, content_end)
                })
            }
            _ => content.span().end,
        }
    }
    root.children.last().and_then(|document| document.body.content.as_ref()).map_or(0, content_end)
}

/// The gap-measurement anchor after an item: a block scalar's span consumes
/// its trailing line breaks (they are part of its VALUE under keep chomping),
...
fn item_gap_anchor(content: Option<&Content<'_>>, end: u32, f: &YamlFormatter<'_, '_>) -> u32 {
    let Some(block) = content.and_then(last_descendant_block_scalar) else { return end };
    // Under keep chomping every trailing newline IS the value; the span end is
    // the correct anchor.
    if block.chomping == Chomping::Keep {
        return end;
    }
    let raw = f.context().source_text().text_for(&to_span(block.span));
    let trimmed = raw.trim_end_matches('\n');
    let total_newlines = raw.len() - trimmed.len();
    // Only the trailing newlines the scalar's own output does NOT consume are
    // the inter-item gap.
    ...
    block.span.start + u32::try_from(trimmed.len() + kept).unwrap_or(0)
}
...
/// Claims pending comments indented strictly deeper than `item_column` as the
/// preceding item's end comments, printed one indent level in (the placement
/// effect of Prettier's `shouldOwnEndComment` + `mappingValue.endComments`,
/// re-derived positionally). Returns the position after the last claimed
/// comment so the caller can keep measuring gaps from it.
fn flush_container_end_comments(
    ...
    loop {
        ...
        if span.end > upper_bound
            ...
            // An end-comment run directly follows its container; other tokens
            // in between mean the comment belongs to a LATER node (a nested
            // collection's unbounded tail flush must not jump over the
            // parent's following items).
...
/// Emits the previous item's same-line trailing comment and container end
/// comments, then the separator to the next item (when one follows).
///
/// Both are skipped after a block scalar value: its span consumes the trailing
/// line breaks (a comment on the following line would LOOK same-line, and none
/// can follow one), and comments under its content region lead the next item
/// instead (Prettier's `shouldOwnEndComment` exclusion).
...
fn write_mapping<'a>(mapping: &'a Mapping<'a>, f: &mut YamlFormatter<'_, 'a>) {
    let depth = f.context().collection_depth();
    depth.set(depth.get() + 1);
    let item_column = column_of(&f.context().source_text(), mapping.span.start);
    let parent_tag_is_set = mapping.props.tag.is_some_and(|tag| {
    ...
    for item in &mapping.children {
        let start = mapping_item_start(item);
        if let Some(prev_end) = prev_end {
            finish_previous_item(item_column, prev_end, prev_blocks_end_comments, Some(start), f);
        }
        prev_end = Some(item_gap_anchor(item.value.content.as_ref(), item.span.end, f));
        // Descendant-aware, like `item_gap_anchor`: a block scalar ending the
        // item at ANY depth consumes the trailing line breaks, so a following
        // own-line comment would otherwise classify as same-line and be
        // absorbed into the scalar's content (changing the parsed value).
        prev_blocks_end_comments =
            item.value.content.as_ref().and_then(last_descendant_block_scalar).is_some();
...
fn write_sequence<'a>(sequence: &'a Sequence<'a>, f: &mut YamlFormatter<'_, 'a>) {
    ...
    for item in &sequence.children {
        ...
        prev_end = Some(item_gap_anchor(item.content.as_ref(), item.span.end, f));
        // Descendant-aware for the same reason as in `write_mapping`.
        prev_blocks_end_comments =
            item.content.as_ref().and_then(last_descendant_block_scalar).is_some();
        ...
        if let Some(content) = &item.content {
            write!(f, align(2, &format_with(|f| write_content(content, f))));
        }
    }
    if let Some(prev_end) = prev_end {
        finish_previous_item(item_column, prev_end, prev_blocks_end_comments, None, f);
